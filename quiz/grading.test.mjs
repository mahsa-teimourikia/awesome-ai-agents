...
test("the quiz contains 28 questions across 6 categories", () => {
  ...
  assert.equal(categories["Tools & Memory"], 9);
...
test("a complete answer key earns 100 percent", () => {
  ...
  assert.equal(result.answeredCount, 28);
  assert.equal(result.correctCount, 28);
