...
## 1. The SOTA Solution: Phase-Specific Context Packets

State-of-the-Art (SOTA) agent architectures do not use a single, monolithic chat history. Instead, they dynamically build a **ContextPacket** representing only the data necessary for the *current phase* of the task.
...
## 2. Implementation: The Context Pipeline
...
4. **Phase Relevance:** Does the current phase (`TRIAGE` vs `INVESTIGATE`) actually require raw tool evidence?
5. **Token Budgeting:** Rank the remaining items by relevance score and drop anything that exceeds the current phase's token limit.
