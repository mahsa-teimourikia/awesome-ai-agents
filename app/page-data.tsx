...
export const curriculumData:Subject[] = [
  ...
  },
  {
    ...
    "description": "Build a policy-controlled MCP host that separates protocol interoperability from identity, authorization, approval, execution, and evidence trust.",
    "time": "120-150 min",
    "outcome": "Implement and evaluate a credential-free enterprise MCP gateway with current-protocol semantics and a tested legacy SDK adapter.",
    "lesson": "MCP standardizes context exchange; the application still owns trust, authority, lifecycle, budgets, validation, and completion.",
    "exercise": "Operate the Northstar gateway across discovery, read-only tools, resources, prompts, approval-gated writes, unknown outcomes, and server quarantine.",
    "failures": ["Discovery mistaken for authorization", "Self-asserted server trust", "Cross-tenant resource access", "Prompt or tool output treated as authority", "Blind retries after unknown writes", "Stale capability snapshots", "Unbounded calls and responses"],
    ...
    "refs": [
      ...
      "curriculum/advanced/13-mcp-model-context-protocol/TOOLS_RESOURCES_PROMPTS.md",
      "curriculum/advanced/13-mcp-model-context-protocol/SECURITY_AND_AUTHORIZATION.md",
      "curriculum/advanced/13-mcp-model-context-protocol/ENTERPRISE_MCP_GATEWAYS.md"
    ],
    "code": "policy.py + lab.py",
    "goals": ["Explain current and legacy MCP lifecycle semantics without coupling the policy architecture to one SDK.","Filter discovered tools, resources, and prompts through current principal, tenant, scope, server, and capability policy.","Protect consequential calls with exact approvals, stable idempotency identity, reconciliation, budgets, and audit.","Treat remote content as evidence, validate its provenance and shape, and keep it outside the authority plane."],
    "quiz": [
      {
  ...
  },
  {
    ...
    "time": "135 min",
    "outcome": "Route, activate, compose, execute, and evaluate versioned skills without confusing package text with authority.",
    "lesson": "A deterministic Northstar control plane separates package metadata from registry trust, current authorization, evidence, and verified completion.",
    "exercise": "Run the incident-analysis vertical slice, test malicious packages and evidence, and extend routing or dependency governance.",
    "failures": ["Ranking before eligibility", "Treating package metadata as permission", "Trusting hashes as approval", "Executing package scripts on the host", "Accepting model confidence as evidence"],
    ...
    "refs": [
      ...
      "curriculum/advanced/14-agent-skills/ANATOMY_OF_A_SKILL.md",
      "curriculum/advanced/14-agent-skills/SKILL_LIBRARIES_AND_ROUTING.md",
      "curriculum/advanced/14-agent-skills/SKILLS_VS_MCP.md"
    ],
    "code": "curriculum/advanced/14-agent-skills/lab.py",
    "goals": ["Separate a framework-neutral skill abstraction from one package format.","Filter ineligible packages before ranking and preserve abstention.","Activate exact versions with least authority and shared budgets.","Validate evidence-bound outputs and keep consequential actions as proposals.","Govern dependencies, sandboxing, provenance, rollout, and emergency revocation."],
    "quiz": [
      {
