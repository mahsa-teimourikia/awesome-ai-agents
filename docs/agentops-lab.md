...
| [06 Stateful investigation graph](../curriculum/intermediate/03-langgraph-state-memory/02_langgraph_state_memory.ipynb) | Stateful agentic workflow | LangGraph | Graph state, conditional routing, confidence loops, thread state, and long-term memory risk |
| [07 Human-in-the-loop and permissions](../curriculum/intermediate/04-human-approval-permissions/03_human_approval_permissions.ipynb) | Bounded action with approval | LangGraph/LangChain HITL concepts | Permission levels, persisted pause state, approval, modification, rejection, and audit records |
| [08 Guardrails and untrusted content](../curriculum/intermediate/05-guardrails-untrusted-content/04_guardrails_untrusted_content.ipynb) | Trust-boundary enforcement | OpenAI Agents SDK guardrail concepts | Treating retrieved content as data, detecting poisoned instructions, and wrapping tools with approval guardrails |
| [09 Agent evaluation: trajectory](../curriculum/intermediate/06-agent-evaluation/05_agent_evaluation.ipynb) | Release-gated agent | Evaluation datasets and traces | Outcome, trajectory, operations, forbidden actions, and cost per successful task |
| [10 Optimize the trajectory](../curriculum/intermediate/07-trajectory-optimization/06_trajectory_optimization.ipynb) | Efficient bounded agent | Trajectory optimization | Shortening reliable paths while preserving correctness, evidence, and support |
