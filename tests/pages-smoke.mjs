...
for (const asset of assets) await access(resolve(outDir, asset.replace(/^\/awsome-ai-agents\//, "")), constants.R_OK);
const javascript = await Promise.all(assets.filter((asset) => asset.endsWith(".js")).map((asset) => readFile(resolve(outDir, asset.replace(/^\/awsome-ai-agents\//, "")), "utf8")));
